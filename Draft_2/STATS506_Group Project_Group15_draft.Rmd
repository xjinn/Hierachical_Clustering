...
Hierarchical clustering can be applied to the situation when underlying data has a hierarchical structure. Hierarchical clustering's time complexity is generally in the order of O(n 2 logn), n being the number of data points. So, when the number of data points is very large, we may not want to rely on it.
...
From the five clusters shown in the figures above, we can see the outcomes of hierarchical clustering have some relationship with geographic locations of the 50 states. For example, most states in the south are divided into one cluster and Alaska is divided into another cluster itself. 
![]()
# References
