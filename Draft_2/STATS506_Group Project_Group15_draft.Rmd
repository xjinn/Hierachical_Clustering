...
We use Elbow Method by package 'factoextra' to determine the number of clusters obtained.
...
To help find the "elbow", we can check the 2nd derivative of the WSS
```python
accele = np.diff(wss, 2) 
```

The gain in explained variance reduces significantly from 3 to 4 to 5. So, optimal number of clusters could be either 4 or 5.
```python
accele
```
